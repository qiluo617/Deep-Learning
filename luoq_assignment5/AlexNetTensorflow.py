...
#
# im3 = (imread("quail227.JPEG")[:,:,:3]).astype(np.float32)
# print(im3.shape)
# im3[:, :, 0], im3[:, :, 2] = im3[:, :, 2], im3[:, :, 0]
#
# im4 = (imread("dog.png")[:,:,:3]).astype(np.float32)
# print(im4.shape)
# im4[:, :, 0], im4[:, :, 2] = im4[:, :, 2], im4[:, :, 0]
#
# im5 = (imread("dog2.png")[:,:,:3]).astype(np.float32)
# print(im5.shape)
# im5[:, :, 0], im5[:, :, 2] = im5[:, :, 2], im5[:, :, 0]

test = (imread("test.png")[:,:,:3]).astype(np.float32)
print(test.shape)
...
# output = sess.run(prob, feed_dict = {x:[im3, im4, im5]})
output = sess.run(prob, feed_dict = {x:[test]})
# Print output:
...
# conv_img = session.run(conv1, feed_dict={x:[im5]})
# plt.title('dog2.png')
# print(conv_img.shape)
# plt.imshow(conv_img[0,0:57,0:57,0])
# plt.show()

# final_layer = session.run(fc8, feed_dict={x:[im1]})
